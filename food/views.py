...
def ingredient_input_view(request):
    ...
    selected_ingredients = user_data.ingredients.all().order_by('name')
    ...
    if request.method == 'POST':
        ...
        if len(name) < 1:
            ...
            try:
                ...
                if not user_data.ingredients.filter(name=name).exists():
                    user_data.ingredients.add(ingredient)
                else:
...
def ingredient_result_view(request):
    session_ingredients = request.session.get('cart_ingredients', [])
    user_data, _ = UserData.objects.get_or_create(user=request.user)
    model_ingredients = user_data.ingredients.all()

    # 모델 기반 재료를 세션 재료 형식으로 맞추기
    model_ingredient_data = [
        {
            'id': ing.id,
            'name': ing.name,
            'image_url': ing.image.url if ing.image else ''
        }
        for ing in model_ingredients
    ]

    # 중복 제거 (이름 기준)
    existing_names = {item['name'] for item in session_ingredients}
    combined = session_ingredients + [item for item in model_ingredient_data if item['name'] not in existing_names]

    return render(request, 'food/ingredient_result.html', {
        'selected_ingredients': combined
    })
...
def add_ingredient_ai(request):
    if request.method == "POST":
        ...
        # 세션 덮어쓰기
        request.session['cart_ingredients'] = session_ingredients
